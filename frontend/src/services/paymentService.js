...
// Fetches the NBS IPS QR code (base64 PNG + raw IPS string) generated by the
// PSP for a QR payment. The mobile banking app scans this to confirm payment.
export const getPaymentQRCode = async (paymentId) => {
  const response = await fetch(`${API_URL}/payments/${paymentId}/qr-code`, {
    headers: { ...authHeader() },
  });
  if (!response.ok) {
    throw new Error(await parseError(response, 'Failed to load QR code'));
  }
  return await response.json();
};
